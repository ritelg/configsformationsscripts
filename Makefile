...
fedora_systemd:
	...
	docker exec -it $(FEDORA_NAME) bash
...
debian_systemd:
	docker run -ti --rm --cap-add NET_ADMIN --cap-add SYS_ADMIN --publish-all=true -v /srv/data:/srv/html -v /sys/fs/cgroup:/sys/fs/cgroup:ro --name $(DEBIAN_NAME) -h $(DEBIAN_NAME) ritelg/debian:10-systemd
